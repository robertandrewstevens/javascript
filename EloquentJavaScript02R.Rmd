---
title: "EloquentJavaScript02R.Rmd"
...
Inside the computer's world, there is only data. That which is not data, does not exist. Although all data is in essence just a sequence of bits [1], and is thus fundamentally alike, every piece of data plays its own role. In R's system, most of this data is neatly separated into things called values. Every value has a type, which determines the kind of role it can play. There are six basic types of values: Numbers, strings, booleans, objects, functions, and undefined values.
...
The number above has 64 bits. Numbers in R always do. This has one important repercussion: There is a limited amount of different numbers that can be expressed. With three decimal digits, only the numbers 0 to 999 can be written, which is 10^3 = 1000 different numbers. With 64 binary digits, 2^64 different numbers can be written. This is a lot, more than 10^19 (a one with nineteen zeroes).

Not all whole numbers below 10^19 fit in a R number though. For one, there are also negative numbers, so one of the bits has to be used to store the sign of the number. A bigger issue is that non-whole numbers must also be represented. To do this, 11 bits are used to store the position of the decimal dot within the number.

That leaves 52 bits [3]. Any whole number less than 2^52, which is over 10^15, will safely fit in a R number. In most cases, the numbers we are using stay well below that, so we do not have to concern ourselves with bits at all. Which is good. I have nothing in particular against bits, but you *do* need a terrible lot of them to get anything done. When at all possible, it is more pleasant to deal with bigger things.
...
The main thing to do with numbers is arithmetic. Arithmetic operations such as addition or multiplication take two number values and produce a new number from them. Here is what they look like in R:
...
```{r}
cat("A newline character is written like \"\\n\".")
```
...
Not all operators are symbols, some are written as words. For example, the **mode** operator, which produces a string value naming the type of the value you give it.

```{r}
mode(4.5)
```

The other operators we saw all operated on two values, **mode** takes only one. Operators that use two values are called binary operators, while those that take one are called unary operators. The minus operator can be used both as a binary and a unary operator:
...
There are also some useful operations that can be applied to boolean values themselves. R supports three logical operators: *and*, *or*, and *not*. These can be used to 'reason' about booleans.
...
How does a program keep an internal state? How does it remember things? We have seen how to produce new values from old values, but this does not change the old values, and the new value has to be immediately used or it will dissipate again. To catch and hold values, R provides a thing called a variable.
...
A lot of the values provided by the standard environment have the type 'function'. A function is a piece of program wrapped in a value. Generally, this piece of program does something useful, which can be evoked using the function value that contains it. For example, the object **cat** holds a function that shows text and numbers. It is used like this:

```{r}
cat("Also, your hair is on fire.\n")
```

Executing the code in a function is called invoking or applying it. The notation for doing this uses parentheses. Every expression that produces a function value can be invoked by putting parentheses after it. The string value between the parentheses is given to the function, which uses it as the text to show in the dialog window. Values given to functions are called parameters or arguments. **cat** needs only one of them, but other functions might need a different number.

---

Showing a dialog window is a side effect. A lot of functions are useful because of the side effects they produce. It is also possible for a function to produce a value, in which case it does not need to have a side effect to be useful. For example, there is a function **max**, which takes two or more arguments and gives back the biggest of the two:

```{r}
cat(max(2, 4), "\n")
```

When a function produces a value, it is said to return it. Because things that produce values are always expressions in R, function calls can be used as a part of bigger expressions:
...
￼As the previous examples show, **cat** can be useful for showing the result of some expression. Clicking away all those little windows can get on one's nerves though, so from now on we will prefer to use a similar function, called **print**, which does not pop up a window, but just writes a value to the output area of the console. **print** is a standard R function, but doesn't have all the formating feature that print methods have in other languages.  If you need more advanced formating, there are other methods in R for that.
...
```{r}
confirm <- readline("Shall we, then?")
```
...
```{r}
cat(readline("Tell us everything you know."))
```
...
```{r}
theNumber = as.numeric(readline("Pick a number"))
cat("Your number is the square root of", theNumber * theNumber, "\n")
```
...
A lot of loops fall into this pattern. For this reason, R, and similar languages, also provide a slightly shorter and more comprehensive form:
...
Note that names that have a special meaning, such as **var**, **while**, and **for** may not be used as variable names. These are called keywords. There are also a number of words which are 'reserved for use' in future versions of R. These are also officially not allowed to be used as variable names, though some browsers do allow them. The full list is rather long:
...
result <- 1
counter <- seq(0, 9)
for(i in counter) result <- result * 2
print(result)
```
...
A program often needs to 'update' a variable with a value that is based on its previous value. For example **counter = counter + 1**. R provides a shortcut for this: **counter += 1**. This also works for many other operators, for example **result \*= 2** to double the value of **result**, or **counter -= 1** to count downwards.
...
```{r}
answer <- readline("You! What is the value of 2 + 2?")
answer <- "4"
if (answer == "4") {
  cat("You must be a genius or something.\n")
} else if (answer == "3" || answer == "5") {
  cat("Almost!\n") 
} else {
  cat("You're an embarrassment.\n")
}
```
...
```{r}
current <- seq(20, 100)
for(i in current) if (i %% 7 == 0) print(i)
```
...
```{r}
cat("You! What is the value of 2 + 2?  You have 3 guesses.\n")
answer <- c(2, 3, 4)
guess <- 0
while (guess < 3) {
  guess <- guess + 1
  if (answer[guess] == "4") {
    cat(answer[guess], "is correct! You must be a genius or something.\n")
    break 
  }
  else if (answer[guess] == "3" || answer[guess] == "5") {
    cat(answer[guess], "is almost correct! Try again.\n")
  }
  else {
    cat(answer[guess], "?!?! You're an embarrassment. Try again.\n", sep = "")
  }
}
```
...
while (guess <= 3) {
  if (value[guess] == "4") {
    cat(value[guess], "is correct! You must be a genius or something.\n") 
    guess <- 4
  } else if (value[guess] == "3" || value[guess] == "5") {
    cat(value[guess], "is almost correct! Try again.\n") 
    guess <- guess + 1
  } else {
    cat(value[guess], "?!?! You're an embarrassment. Try again.\n", sep = "")
    guess <- guess + 1
  }
}
```
...
All these give the value **TRUE**. When comparing values that have different types, R uses a complicated and confusing set of rules. I am not going to try to explain them precisely, but in most cases it just tries to convert one of the values to the type of the other value. However, when **null** or **undefined** occur, it only produces **TRUE** if both sides are **null** or **undefined**.
...
```{r}
print(identical(NULL, NaN)) 
print(identical(FALSE, 0)) 
print(identical("", 0))
print(identical("5", 5))
```
...
maybeNull <- "string"
if (is.character(maybeNull)) cat("maybeNull has a value\n")
```
...
There are some other situations that cause automatic type conversions to happen. If you add a non-string value to a string, the value is automatically converted to a string before it is concatenated. If you multiply a number and a string, R tries to make a number out of the string.

```{r}
print(paste("Apollo", 5))
print(paste(NULL, "ify")) 
print(rep("5", 5))
print(rep("strawberry", 5))
```
...
```{r}
FALSE || is.character("I'm happening!")
TRUE || is.character("Not me.")
```
