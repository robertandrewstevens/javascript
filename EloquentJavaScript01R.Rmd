---
title: "EloquentJavaScript01R.Rmd"
...
Fortunately, as an effect of developments in the World Wide Web and open source software, it so happens that every computer user with internet access can obtain an environment for programming R. In today's spirit of not bothering the user with technical details, it is kept well hidden, but a web-page can make it accessible, and use it as a platform for learning to program. That is what this (hyper-)book tries to do.
...
The moral of this story, then, is that the same program can be expressed in long and short, unreadable and readable ways. The first version of the program was extremely obscure, while this last one is almost English: **print** the **sum** of the **seq** (sequence) of numbers from **1** to **10**. (We will see in later chapters how to build things like **sum** and **seq**.)
...
R is the language that is, at the moment, mostly being used to do all kinds of clever and advanced things with statistics. Some people claim that R will become an important language for other tasks too. I am unsure whether that is true, but if you are interested in statistical programming, R is definitely a useful language to learn. Even if you do not end up doing much statistics programming, the mind-bending programs I will show you in this book will always stay with you, haunt you, and influence the programs you write in other languages.

There are those who will say *terrible* things about R. Many of these things are true. When I was for the first time required to write something in R, I quickly came to enchanted by the language. It would accept almost anything I typed, but interpret it in a way that was completely different from what I meant. This had a lot to do with the fact that I did not have a clue what I was doing, but there is also a real issue here: R is ridiculously liberal in what it allows. The idea behind this design was that it would make programming in R easier for beginners. In actuality, it mostly makes finding problems in your programs harder, because the system will not point them out to you.

However, the flexibility of the language is also an advantage. It leaves space for a lot of techniques that are impossible in more rigid languages, and it can be used to overcome some of R's shortcomings. After learning it properly, and working with it for a while, I have really learned to love this language.
...
Most chapters in this book contain quite a lot of code[1]. In my experience, reading and writing code is an important part of learning to program. Try to not just glance over these examples, but read them attentively and understand them. This can be slow and confusing at first, but you will quickly get the hang of it. The same goes for the exercises. Don't assume you understand them until you've actually written a working solution.

Because of the way the R user community works, it is always possible to look at the R programs that people put in their web-pages. This can be a good way to learn how some things are done. Because most R programmers are not 'professional' programmers or use R pragmatically to obtain statistical results, they never properly learned it, and so a lot of the code you can find like this is of a very bad quality. When learning from ugly or incorrect code, the ugliness and confusion will propagate into your own code, so be careful who you learn from.  Try to identify and learn from the masters.

```
```

To allow you to try out programs, both the examples and the code you write yourself, this book makes use of something called a Integrated Development Environment (IDE). RStudio is a propular IDE for R programming. It contains two important elements. There is the script window where you can copy-and-paste or type in large pieces of R code. Below that, there is console window, which is used to enter a line of R code and show error messages and things that programs print out. Try typing in a number, and pressing the enter key to run what you typed. If the text you typed produced something meaningful, it will be shown in the next line of the console window. Now try typing **wrong!**, and press enter again. The output window will show an error message. You can use the arrow-up and arrow-down keys to go back to previous commands that you typed.

For bigger pieces of code, those that span multiple lines and which you want to keep around for a while, the script window should be used. The 'Run' button is used to execute programs written in this window. It is possible to have multiple programs open at the same time. Use the 'New File' and 'Open File...' menu options to add a new program (empty or from a file on the web). When there is more than one open program, then select the tab to choose which one is being shown. The 'Close' button represented by 'x', as you might expect, closes a program.

Run lines of code in the tabs by clicking the 'Run' button with an arrow in their top-right corner. The example we saw earlier looked like this:
...
Run it by highlighting all of the lines of code and clicking the Run button. This is loads the program into the console. Do not hesitate to modify it and try out the result. The worst that could happen is that you create an endless loop. An endless loop is what you get when the condition of the while never becomes false, for example if you choose to add 0 instead of 1 to the count variable. Now the program will run forever.

Fortunately, IDEs keep an eye on the programs running inside them. Whenever one of them is taking suspiciously long to finish, locate and select the 'Stop' button.

```
```

In some later chapters, we will build example programs that consist of many blocks of code. Often, you have to run every one of them for the program to work. When reading a chapter, try to run every block of code you come across, especially those that 'define' something new (you will see what that means in the next chapter).

It is, of course, possible that you can not read a chapter in one sitting. This means you will have to start halfway when you continue reading, but if you don't run all the code starting from the top of the chapter, some things might not work. By highlighting the entire block of code and then pressing the 'Run' button, all lines of code in the block will be run as well, so when you start in the middle of a chapter, select all code before that, and everything should work as expected.

```
```

Finally, the are often better ways of doing or explaining something, and if you have suggestions, send them me, the author. If you have a comment, or you find a passage ridiculously confusing, or you just spot a spelling error, tell me about it.
